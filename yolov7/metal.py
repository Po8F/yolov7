...
def train(hyp, opt, device, tb_writer=None):
    ...
    # DP mode
    if mps_enabled and rank == -1 and torch.mps.device_count() > 1:
        model = torch.nn.DataParallel(model)

    # SyncBatchNorm
    if opt.sync_bn and mps_enabled and rank != -1:
    ...
    model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device).float() * nc  # attach class weights
    model.names = names
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                mem = '%.3gG' % (
                    torch.mps.current_allocated_memory() / 1E9 if torch.backends.mps.is_available() else 0)  # (GB)
...
if __name__ == '__main__':
    ...
    if torch.backends.mps.is_available():
        ...
        ##torch.backends.mps.set_per_process_memory_growth(True)
    else:
        device = select_device(opt.device, batch_size=opt.batch_size)
        print("Using CUDA/CPU device")

    if opt.local_rank != -1:
        assert torch.mps.device_count() > opt.local_rank
        torch.mps.set_device(opt.local_rank)
        device = torch.device('mps', opt.local_rank)
        dist.init_process_group(backend='gloo', init_method='env://')  # distributed backend
        assert opt.batch_size % opt.world_size == 0, '--batch-size must be multiple of MPS device count'
        ...
        for _ in range(300):  # generations to evolve
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                ...
                if parent == 'single' or len(x) == 1:
                    ...
                    x = random.choices(range(n), weights=w)[0]  # weighted selection
